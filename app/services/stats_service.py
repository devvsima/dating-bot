...
from database.models import Match, Profile, Referal, User
...
class StatsService:
    ...
    async def get_registration_data(session: AsyncSession) -> list:
        ...
        stmt = (
            ...
            .where(User.created_at >= days_ago)  # Фильтрация по дате
            .order_by(User.created_at.asc())  # Сортировка по дате регистрации
        )
    ...
    async def profile_stats(session: AsyncSession) -> dict:
        ...
        stmt = select(
            ...
            func.avg(Profile.age).label("average_age"),
        )
        result = await session.execute(stmt)
        stats = result.fetchone()
        ...
        city_stmt = (
            select(Profile.city, func.count(Profile.city).label("city_count"))
    ...
    async def referral_stats(session: AsyncSession) -> dict:
        ...
        total_stmt = select(func.count(Referal.id))
        total_result = await session.execute(total_stmt)
        total_referrals = total_result.scalar() or 0

        # Получаем статистику по источникам
        source_stmt = (
            select(Referal.source, func.count(Referal.id).label("count"))
            .group_by(Referal.source)
            .order_by(desc("count"))
        )
