...
class SearchService:
    ...
    async def search_profiles(
        ...
        dynamic_age_range = SearchService.calculate_age_range(profile.age)
        ...
        while current_distance <= max_distance and len(found_profiles) < min_profiles:
                ...
                func.acos(
                    func.greatest(
                        func.least(
                            ...
                            * func.cos(func.radians(Profile.latitude))
                            * func.cos(
                                func.radians(Profile.longitude) - func.radians(profile.longitude)
                            )
            ...
            stmt = (
                select(Profile.id, distance_expr.label("distance"))
                .where(
                    and_(
                        ...
                        or_(
                            profile.gender == Profile.find_gender,
                            Profile.find_gender == "all",
                        ),
